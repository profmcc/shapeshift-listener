#!/usr/bin/env python3
"""
ShapeShift Affiliate Fee Tracker - Robust Version
...
# Configuration
DB_PATH = 'shapeshift_affiliate_fees_robust.db'
...
def setup_database():
    ...
    conn.commit()
    conn.close()
    logger.info("Robust affiliate fee database setup complete")
...
def get_block_range_for_week(chain_id: int) -> List[Tuple[int, int]]:
    ...
    chunk_size = 500  # Even smaller chunks for rate limiting
    ranges = []
...
def main():
    """Main function to run the robust affiliate fee listener"""
    logger.info("Starting ShapeShift affiliate fee tracker (robust)")
    ...
    logger.info("Robust affiliate fee listener run complete!")
    ...
    logger.info(f"Robust Affiliate Fee Summary:")
