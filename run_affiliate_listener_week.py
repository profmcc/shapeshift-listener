#!/usr/bin/env python3
"""
ShapeShift Affiliate Fee Tracker - Listener Runner
Fetches all affiliate fee events from the last week and stores them in a database.
Focuses only on CowSwap, 0x Protocol, and Portals affiliate revenue.
"""
...
def setup_database():
    ...
    # Summary table for daily affiliate revenue
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS daily_affiliate_revenue (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            date TEXT,
            chain_id INTEGER,
            protocol TEXT,
...
def fetch_cowswap_affiliate_trades(chain_id: int, start_block: int, end_block: int):
    ...
    logger.info(f"Fetching CowSwap affiliate trades on chain {chain_id}")
    ...
    try:
        ...
        logger.info(f"Found {affiliate_trades} CowSwap affiliate trades out of {len(logs)} total trades")
        
    except Exception as e:
        logger.error(f"Error fetching CowSwap affiliate trades: {e}")
...
def fetch_zerox_affiliate_fees(chain_id: int, start_block: int, end_block: int):
    ...
    logger.info(f"Fetching 0x affiliate fees on chain {chain_id}")
    ...
    try:
        ...
        logger.info(f"Found {affiliate_fees} 0x affiliate fee payments out of {len(logs)} total interactions")
        
    except Exception as e:
        logger.error(f"Error fetching 0x affiliate fees: {e}")
...
def fetch_portals_affiliate_events(chain_id: int, start_block: int, end_block: int):
    ...
    logger.info(f"Fetching Portals affiliate events on chain {chain_id}")
    ...
    try:
        ...
        logger.info(f"Found {affiliate_events} Portals affiliate events out of {len(logs)} total events")
        
    except Exception as e:
        logger.error(f"Error fetching Portals affiliate events: {e}")
    
    conn.commit()
    conn.close()

def calculate_daily_revenue():
    """Calculate daily affiliate revenue summary"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # Get all affiliate fees from the last week
    cursor.execute('''
        SELECT 
            date(created_at) as date,
            chain_id,
            'CowSwap' as protocol,
            sell_token as token_address,
            fee_amount as amount
        FROM cowswap_affiliate_trades
        ...
        SELECT 
            date(created_at) as date,
            chain_id,
            '0x Protocol' as protocol,
            affiliate_fee_token as token_address,
            affiliate_fee_amount as amount
        FROM zerox_affiliate_fees
        WHERE created_at >= date('now', '-7 days')
        ...
        SELECT 
            ...
            input_amount as amount
        FROM portals_affiliate_events
        WHERE created_at >= date('now', '-7 days')
    ''')
    
    results = cursor.fetchall()
    
    # Group by date, chain, protocol, and token
    daily_revenue = {}
    
    for row in results:
        date, chain_id, protocol, token_address, amount = row
        
        key = (date, chain_id, protocol, token_address)
        if key not in daily_revenue:
            daily_revenue[key] = 0
        
        daily_revenue[key] += int(amount)
    
    # Insert into daily revenue table
    for (date, chain_id, protocol, token_address), total_amount in daily_revenue.items():
        cursor.execute('''
            INSERT INTO daily_affiliate_revenue 
            (date, chain_id, protocol, token_address, total_amount)
            VALUES (?, ?, ?, ?, ?)
        ''', (date, chain_id, protocol, token_address, str(total_amount)))
    
    conn.commit()
    conn.close()
    
    logger.info(f"Calculated daily revenue for {len(daily_revenue)} combinations")

def main():
    """Main function to run the affiliate fee listener for the last week"""
    logger.info("Starting ShapeShift affiliate fee tracker")
    ...
    cursor.execute("SELECT COUNT(*) FROM daily_affiliate_revenue")
    ...
    logger.info(f"  Daily revenue entries: {daily_count}")
