...
# Chainflip Extension v3 Installation Script
# This script helps install and configure the extension with improved error handling
...
# Extension details
EXTENSION_NAME="Chainflip Affiliate Tracker v3"
...
check_manifest() {
    ...
    # Check version
    if grep -q '"version": "3.0.0"' "$EXTENSION_DIR/manifest.json"; then
        echo -e "${GREEN}✅ Version 3.0.0 confirmed${NC}"
    else
        echo -e "${YELLOW}⚠️  Version may not be 3.0.0${NC}"
    fi
}
...
# Check for v3 specific improvements
check_v3_features() {
    echo -e "\n${BLUE}🔍 Checking v3 features...${NC}"
    
    # Check for connection handling improvements
    if grep -q "connectionRetries\|maxRetries\|retryDelay" "$EXTENSION_DIR/content.js"; then
        echo -e "${GREEN}✅ Connection retry logic found${NC}"
    else
        echo -e "${YELLOW}⚠️  Connection retry logic not found${NC}"
    fi
    
    # Check for local storage fallback
    if grep -q "saveToLocalStorage\|loadFromLocalStorage" "$EXTENSION_DIR/content.js"; then
        echo -e "${GREEN}✅ Local storage fallback found${NC}"
    else
        echo -e "${YELLOW}⚠️  Local storage fallback not found${NC}"
    fi
    
    # Check for enhanced error handling
    if grep -q "handleConnectionError\|fallbackToLocalMode" "$EXTENSION_DIR/content.js"; then
        echo -e "${GREEN}✅ Enhanced error handling found${NC}"
    else
        echo -e "${YELLOW}⚠️  Enhanced error handling not found${NC}"
    fi
}
...
show_instructions() {
    ...
    echo -e "\n${BLUE}🔧 v3 Troubleshooting Tips:${NC}"
    echo -e "${YELLOW}• If you see 'Receiving end does not exist' errors, v3 will automatically handle them${NC}"
    echo -e "${YELLOW}• The extension will fall back to local mode if background script fails${NC}"
    echo -e "${YELLOW}• Check the connection status in the popup to monitor health${NC}"
    echo -e "${YELLOW}• Use the refresh button to manually retry connections${NC}"
}
...
main() {
    ...
    echo -e "\n${BLUE}🚀 v3 is ready to handle connection issues automatically!${NC}"
}
