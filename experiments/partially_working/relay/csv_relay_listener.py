#!/usr/bin/env python3
"""
CSV Relay Listener - Centralized Configuration Version
=====================================================

This listener monitors Relay transactions for ShapeShift affiliate fees.
It uses the centralized configuration system and saves data to CSV files.

⚠️  CRITICAL ISSUE: Current detection method is fundamentally flawed
   - Looking for AffiliateFee events that don't exist
   - Need to investigate actual Relay affiliate mechanism
   - This listener will not find transactions until fixed
...
class CSVRelayListener:
    ...
    def __init__(self):
        ...
        self.logger.info("✅ CSVRelayListener initialized successfully")
        ...
        self.logger.warning("⚠️  CRITICAL: Current detection method is flawed - needs investigation")
            ...
            try:
                ...
                if chain_config and 'rpc_url' in chain_config:
                    ...
                    if w3.is_connected():
                        ...
                        contract_address = self.config.get_contract_address('relay', chain_name)
    ...
    def _init_csv_structure(self):
        ...
        transactions_path = os.path.join(self.transactions_dir, 'relay_transactions.csv')
        if not os.path.exists(transactions_path):
            headers = [
                ...
                'relay_type', 'solver_address', 'created_at'
            ]
            with open(transactions_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(headers)
            self.logger.info(f"✅ Created Relay transactions CSV: {transactions_path}")
        
        # Initialize block tracker CSV
        block_tracker_path = os.path.join(self.block_tracking_dir, 'relay_block_tracker.csv')
        if not os.path.exists(block_tracker_path):
            ...
            with open(block_tracker_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(headers)
            self.logger.info(f"✅ Created Relay block tracker CSV: {block_tracker_path}")
...
# =============================================================================
    ...
    def save_transactions_to_csv(self, transactions: List[Dict[str, Any]]):
        """Save Relay transactions to CSV file"""
        if not transactions:
            return
        ...
        self.logger.info(f"✅ Saved {len(transactions)} Relay transactions to CSV")
...
# =============================================================================
    ...
    def process_chain(self, chain_name: str, start_block: int, end_block: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for log in logs:
                try:
                    transaction = self._parse_relay_log(w3, log, chain_name)
                    if transaction:
    ...
    def _parse_relay_log(self, w3: Web3, log: Dict, chain_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            transaction = {
                ...
                'relay_type': 'relay',
                'solver_address': '0x0000000000000000000000000000000000000000',
            ...
            self.logger.error(f"❌ Error parsing Relay log: {e}")
            return None
    ...
    def _check_affiliate_involvement(self, receipt: Dict, tx: Dict) -> Optional[str]:
        ...
        # ⚠️  CRITICAL ISSUE: This method is fundamentally flawed
        # Current implementation looks for AffiliateFee events that don't exist
        # Need to investigate actual Relay affiliate mechanism
    ...
    def _extract_volume_and_fees(self, w3: Web3, receipt: Dict, transaction: Dict):
        ...
        try:
            ...
            # For now, set affiliate fee to 0 (would need specific Relay logic)
...
# =============================================================================
    ...
    def run_listener(self, chains: Optional[List[str]] = None, max_blocks: Optional[int] = None):
        ...
        self.logger.warning("⚠️  CRITICAL: Relay listener has fundamental detection issues")
        self.logger.warning("⚠️  This listener will not find transactions until fixed")
        self.logger.info(f"🚀 Starting Relay listener for chains: {chains}")
        ...
        self.logger.info(f"🎯 Relay listener completed. Total transactions: {total_transactions}")
        if total_transactions == 0:
            self.logger.warning("⚠️  No transactions found - this confirms the detection issue")
...
def main():
    ...
    try:
        print("⚠️  WARNING: Relay listener has fundamental detection issues")
        print("⚠️  This listener will not find transactions until fixed")
        print("⚠️  See handoff documentation for details")
        ...
        if total_transactions == 0:
            print(f"\n⚠️  No transactions found - this confirms the detection issue")
            print(f"⚠️  Need to investigate actual Relay affiliate mechanism")
        
    except Exception as e:
        logging.error(f"❌ Error running Relay listener: {e}")
        raise
