#!/usr/bin/env python3
"""
Weekly DAO FOX Pool Activity Analysis - Real On-Chain Data
Fetches actual blockchain data for DAO liquidity events.
"""
...
# Recent time periods to analyze (last 3 months)
RECENT_START = datetime.now() - timedelta(days=90)
RECENT_END = datetime.now()

def get_working_web3_connection(chain_name: str) -> Web3:
    """Get working Web3 connection with fallback RPCs"""
    endpoints = RPC_ENDPOINTS[chain_name]
    ...
    for endpoint in endpoints:
        try:
            ...
            logger.info(f"Connected to {chain_name} via {endpoint}")
            return w3
        except Exception as e:
            logger.warning(f"Failed to connect to {endpoint}: {e}")
            continue
    
    raise Exception(f"Could not connect to any RPC for {chain_name}")

def get_block_by_timestamp_robust(w3: Web3, target_timestamp: int) -> int:
    """Get block number with robust error handling"""
    try:
        ...
        # Use a more efficient approach - estimate block number
        blocks_per_day = 7200  # Approximate for Ethereum
        days_diff = (latest_timestamp - target_timestamp) / 86400
        estimated_blocks = int(days_diff * blocks_per_day)
        estimated_block = max(0, latest_block['number'] - estimated_blocks)
        
        # Verify the estimated block
        try:
            test_block = w3.eth.get_block(estimated_block)
            if abs(test_block['timestamp'] - target_timestamp) < 3600:  # Within 1 hour
                return estimated_block
        except:
            pass
        
        # Fallback to latest block if estimation fails
        return latest_block['number']
        
    except Exception as e:
        logger.error(f"Error getting block by timestamp: {e}")
        # Return a reasonable fallback
        return w3.eth.get_block('latest')['number'] - 1000

def fetch_events_with_retry(w3: Web3, pool_address: str, start_block: int, end_block: int, dao_address: str) -> List[Dict]:
    """Fetch events with retry logic and smaller chunks"""
    events = []
    ...
    # Use very small chunks to avoid rate limits
    chunk_size = 500
    max_retries = 3
            ...
            try:
                ...
                for log in mint_logs:
                    try:
                        ...
                        logger.debug(f"Error processing mint log: {e}")
                        continue
                ...
                for log in burn_logs:
                    try:
                        ...
                        logger.debug(f"Error processing burn log: {e}")
                        continue
                ...
                logger.warning(f"Attempt {attempt + 1} failed for blocks {chunk_start}-{chunk_end}: {e}")
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)  # Exponential backoff
                else:
                    logger.error(f"Failed to fetch blocks {chunk_start}-{chunk_end} after {max_retries} attempts")
...
def analyze_recent_activity(chain_name: str, pool_info: Dict, dao_address: str) -> List[Dict]:
    """Analyze recent DAO activity for a specific pool"""
    logger.info(f"Analyzing {pool_info['pool_name']} on {chain_name}")
    
    try:
        w3 = get_working_web3_connection(chain_name)
        
        # Get block numbers for recent period
        start_timestamp = int(RECENT_START.timestamp())
        end_timestamp = int(RECENT_END.timestamp())
        
        start_block = get_block_by_timestamp_robust(w3, start_timestamp)
        end_block = get_block_by_timestamp_robust(w3, end_timestamp)
        
        logger.info(f"Analyzing blocks {start_block} to {end_block}")
        
        # Fetch events
        events = fetch_events_with_retry(w3, pool_info['pool_address'], start_block, end_block, dao_address)
        
        logger.info(f"Found {len(events)} events for {pool_info['pool_name']}")
        
        # Group events by week
        weekly_data = []
        current_week_start = RECENT_START.replace(hour=0, minute=0, second=0, microsecond=0)
        
        while current_week_start <= RECENT_END:
            week_end = current_week_start + timedelta(days=7)
            if week_end > RECENT_END:
                week_end = RECENT_END
        ...
        logger.error(f"Error analyzing {pool_info['pool_name']} on {chain_name}: {e}")
        return []
...
def create_color_table(weekly_results: List[Dict]) -> None:
    ...
    if df.empty:
        print("\n" + "="*100)
        print("WEEKLY DAO NET LIQUIDITY CHANGES - RECENT ACTIVITY")
        print("(Green = Additions, Red = Removals)")
        print("="*100)
        print("\nNo DAO liquidity events found in the recent period.")
        print("This could mean:")
        print("- The DAO has not made any liquidity changes recently")
        print("- The events are not accessible through current RPC endpoints")
        print("- The DAO addresses may not be actively trading")
        return
...
def save_results_to_db(weekly_results: List[Dict]) -> None:
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS weekly_dao_activity_real (
    ...
    # Clear existing data
    cursor.execute('DELETE FROM weekly_dao_activity_real')
    
    # Insert new data
    for result in weekly_results:
        cursor.execute('''
            INSERT INTO weekly_dao_activity_real 
...
def main():
    ...
    logger.info("Starting Real On-Chain DAO FOX Pool Activity Analysis")
    logger.info(f"Analyzing period: {RECENT_START.strftime('%Y-%m-%d')} to {RECENT_END.strftime('%Y-%m-%d')}")
    ...
    for pool in FOX_POOLS:
        ...
        weekly_data = analyze_recent_activity(chain_name, pool, dao_address)
    ...
    if all_weekly_results:
        ...
        logger.info("Real on-chain analysis complete!")
    else:
        logger.warning("No real on-chain data collected")
        print("\n" + "="*80)
        print("NO REAL DATA FOUND")
        print("="*80)
        print("The analysis found no DAO liquidity events in the recent period.")
        print("This could indicate:")
        print("- The DAO has not been actively trading FOX pools recently")
        print("- The DAO addresses may not be the correct ones")
        print("- The events may require different RPC endpoints or indexing")
