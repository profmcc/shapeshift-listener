#!/usr/bin/env python3
"""
ShapeShift LP Position Tracker - Chunked Listener
Fetches all liquidity position events in small chunks to avoid RPC limits.
"""
...
def fetch_events_in_chunks(chain_id: int, start_block: int, end_block: int, event_type: str):
    ...
    for pool_address, pool_name in config.get('uniswap_v3_pools', []):
        logger.info(f"Fetching {event_type} events for {pool_name} on chain {chain_id}")
        ...
        while current_block < end_block:
            ...
            try:
                logs = w3.eth.get_logs({
                    ...
                    'topics': [LP_EVENT_SIGNATURES[event_type]],
                    'fromBlock': current_block,
                    'toBlock': chunk_end
                })
                
                for log in logs:
                    if event_type == 'increase_liquidity':
                    ...
                    elif event_type == 'decrease_liquidity':
                ...
                logger.info(f"Processed blocks {current_block}-{chunk_end}: {len(logs)} {event_type} events")
                
            except Exception as e:
                logger.error(f"Error fetching {event_type} events for {pool_name} blocks {current_block}-{chunk_end}: {e}")
    ...
    conn.commit()
    conn.close()
    logger.info(f"Total {event_type} events: {total_events}")
...
def main():
    """Main function to run the chunked LP listener"""
    logger.info("Starting ShapeShift LP position tracker (chunked)")
    ...
    for chain_id in LP_CONTRACTS.keys():
        ...
        try:
            # Get block range for last 30 days (more manageable)
            start_block, end_block = get_block_range_for_period(chain_id, 30)
            
            # Fetch events in chunks
            fetch_events_in_chunks(chain_id, start_block, end_block, 'increase_liquidity')
            fetch_events_in_chunks(chain_id, start_block, end_block, 'decrease_liquidity')
            fetch_events_in_chunks(chain_id, start_block, end_block, 'collect')
